...
class JoinTables(Table):
    ...
    def compute(self):
        ...
        def get_column_idx(table, prefix):
            ...
            col_idx = None
            if self.hasInputFromPort(col_name_port):
                col_name = self.getInputFromPort(col_name_port)
                try:
            ...
            if self.hasInputFromPort(col_idx_port):
                port_col_idx = self.getInputFromPort(col_idx_port)
...
class ProjectTable(Table):
    ...
    def compute(self):
        ...
        if self.hasInputFromPort('column_indexes'):
            column_indexes = self.getInputFromPort('column_indexes')
        if self.hasInputFromPort('column_names'):
            indexes = []
            names = self.getInputFromPort('column_names')
            for name in names:
        ...
        elif self.hasInputFromPort('column_indexes'):
...
class SelectFromTable(Table):
    ...
    def compute(self):
        ...
        if self.hasInputFromPort('str_expr'):
            (col, comparer, val) = self.getInputFromPort('str_expr')
        elif self.hasInputFromPort('float_expr'):
            (col, comparer, val) = self.getInputFromPort('float_expr')
        else:
